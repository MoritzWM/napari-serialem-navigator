...
def reader_function(path):
    ...
    for item in nav.items:
        ...
        if len(ptsx) != len(ptsy):
            raise ValueError(
                f"PtsX and PtsY have unequal length in item {item.id}"
            )
        if len(ptsx) == 1:
    ...
    return [
        (
            ...
            dict(
                ...
                properties=dict(polygon_name=polygon_names),
            ),
            "shapes",
        ),
        (
            point_coords,
            dict(
                name=f"{path.stem} points",
                text="{point_name}",
                properties=dict(point_name=point_names),
            ),
            "points",
        ),
    ]
